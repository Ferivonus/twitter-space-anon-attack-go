...
func worker(id, port int, driverPath, spaceURL string, wg *sync.WaitGroup) {
	...
	// WebDriver kabiliyetleri (ses kapalı mod için --mute-audio eklendi)
...
func main() {
	// chromedriver.exe, çalışma dizininde veya exe'nin yanında aranacak
	cwd, err := os.Getwd()
	if err != nil {
		log.Fatalf("Çalışma dizini alınamadı: %v", err)
	}
	// Öncelikle çalışma dizininde ara
	driverPath := filepath.Join(cwd, "chromedriver.exe")
	if _, err := os.Stat(driverPath); os.IsNotExist(err) {
		// yoksa executable yanındaki dizine bak
	...
	if _, err := os.Stat(driverPath); err != nil {
		log.Fatalf("chromedriver.exe bulunamadı: %v", err)
	}
